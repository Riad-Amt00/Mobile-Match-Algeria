'use client'

import { useState } from 'react'
import Link from 'next/link'
import { Zap, Wifi, Phone, MessageSquare, TrendingUp, CheckCircle, AlertCircle, ArrowLeft } from 'lucide-react'
import { formatDA, formatData, formatMinutes, getOperatorColor } from '@/lib/utils'

interface Recommendation {
  offer: any
  score: number
  savings: number
  matchReasons: string[]
  mismatches: string[]
}

export default function RecommendPage() {
  const [step, setStep] = useState(1)
  const [budget, setBudget] = useState(2000)
  const [dataGB, setDataGB] = useState(20)
  const [voiceMinutes, setVoiceMinutes] = useState(100)
  const [smsCount, setSmsCount] = useState(50)
  const [type, setType] = useState('any')
  const [network, setNetwork] = useState('any')
  const [loading, setLoading] = useState(false)
  const [results, setResults] = useState<Recommendation[]>([])

  async function handleSubmit() {
    setLoading(true)
    try {
      const res = await fetch('/api/recommendations', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ budget, dataGB, voiceMinutes, smsCount, type, network }),
      })
      const data = await res.json()
      setResults(data.recommendations || [])
      setStep(2)
    } catch {
      setResults([])
    } finally {
      setLoading(false)
    }
  }

  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-dark)', padding: '2rem 1.5rem' }}>
      <div style={{ maxWidth: 720, margin: '0 auto' }}>
        {/* Header */}
        <Link href="/" style={{
          display: 'inline-flex', alignItems: 'center', gap: 6,
          color: 'var(--text-secondary)', textDecoration: 'none', fontSize: 14,
          marginBottom: '2rem',
        }}>
          <ArrowLeft size={16}/> Retour
        </Link>

        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          <div style={{
            width: 56, height: 56, borderRadius: 16, margin: '0 auto 1rem',
            background: 'linear-gradient(135deg, #4f7fff, #7c3aed)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}>
            <Zap size={28} color="white"/>
          </div>
          <h1 className="section-title">Moteur de Recommandation</h1>
          <p className="section-subtitle">
            Répondez à quelques questions et obtenez vos <strong style={{ color: '#6b93ff' }}>3 meilleures offres</strong> personnalisées
          </p>
        </div>

        {step === 1 && (
          <div className="glass" style={{ borderRadius: 20, padding: '2rem' }}>
            <h2 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '1.75rem' }}>
              Votre profil de consommation
            </h2>

            {/* Budget */}
            <div style={{ marginBottom: '1.75rem' }}>
              <label style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, fontWeight: 600, marginBottom: 10 }}>
                <span>💰 Budget mensuel</span>
                <span className="gradient-text">{formatDA(budget)}</span>
              </label>
              <input type="range" min={100} max={8000} step={100} value={budget}
                onChange={e => setBudget(+e.target.value)}
                style={{ width: '100%', accentColor: '#4f7fff' }}
              />
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--text-secondary)', marginTop: 4 }}>
                <span>100 DA</span><span>8 000 DA</span>
              </div>
            </div>

            {/* Data */}
            <div style={{ marginBottom: '1.75rem' }}>
              <label style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, fontWeight: 600, marginBottom: 10 }}>
                <span><Wifi size={14} style={{ display: 'inline', marginRight: 6 }}/>Volume data mensuel</span>
                <span className="gradient-text">{dataGB} GB</span>
              </label>
              <input type="range" min={0} max={200} step={1} value={dataGB}
                onChange={e => setDataGB(+e.target.value)}
                style={{ width: '100%', accentColor: '#4f7fff' }}
              />
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--text-secondary)', marginTop: 4 }}>
                <span>Peu</span><span>200 GB</span>
              </div>
            </div>

            {/* Voice */}
            <div style={{ marginBottom: '1.75rem' }}>
              <label style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, fontWeight: 600, marginBottom: 10 }}>
                <span><Phone size={14} style={{ display: 'inline', marginRight: 6 }}/>Minutes d'appels / mois</span>
                <span className="gradient-text">{voiceMinutes === 500 ? 'Illimité' : `${voiceMinutes} min`}</span>
              </label>
              <input type="range" min={0} max={500} step={10} value={voiceMinutes}
                onChange={e => setVoiceMinutes(+e.target.value)}
                style={{ width: '100%', accentColor: '#4f7fff' }}
              />
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--text-secondary)', marginTop: 4 }}>
                <span>Peu</span><span>Illimité</span>
              </div>
            </div>

            {/* Type */}
            <div style={{ marginBottom: '1.75rem' }}>
              <label style={{ fontSize: 14, fontWeight: 600, display: 'block', marginBottom: 10 }}>
                Type d'offre préféré
              </label>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {[
                  { id: 'any', label: '✨ Peu importe' },
                  { id: 'PREPAID', label: '💳 Prépayé' },
                  { id: 'POSTPAID', label: '📋 Postpayé' },
                  { id: 'DATA_ONLY', label: '🌐 Internet' },
                ].map(t => (
                  <button key={t.id} onClick={() => setType(t.id)}
                    className={`filter-pill ${type === t.id ? 'active' : ''}`}>
                    {t.label}
                  </button>
                ))}
              </div>
            </div>

            {/* Network */}
            <div style={{ marginBottom: '2rem' }}>
              <label style={{ fontSize: 14, fontWeight: 600, display: 'block', marginBottom: 10 }}>
                Réseau souhaité
              </label>
              <div style={{ display: 'flex', gap: 8 }}>
                {['any', '4G', '5G'].map(n => (
                  <button key={n} onClick={() => setNetwork(n)}
                    className={`filter-pill ${network === n ? 'active' : ''}`}>
                    {n === 'any' ? '🔌 Tous' : n}
                  </button>
                ))}
              </div>
            </div>

            <button
              onClick={handleSubmit}
              disabled={loading}
              className="btn-primary"
              style={{ width: '100%', padding: '0.875rem', fontSize: 16, borderRadius: 12, position: 'relative' }}
            >
              {loading ? 'Analyse en cours...' : '🎯 Trouver mes meilleures offres'}
            </button>
          </div>
        )}

        {step === 2 && (
          <div>
            <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
              <h2 style={{ fontSize: '1.5rem', fontWeight: 700 }}>Vos <span className="gradient-text">3 meilleures offres</span></h2>
              <p style={{ color: 'var(--text-secondary)', fontSize: 14, marginTop: 6 }}>
                Classées par score de correspondance
              </p>
            </div>

            {results.length === 0 ? (
              <div className="glass" style={{ padding: '3rem', textAlign: 'center', borderRadius: 16 }}>
                <p style={{ color: 'var(--text-secondary)' }}>Aucune offre ne correspond à vos critères. Ajustez votre budget.</p>
                <button onClick={() => setStep(1)} className="btn-primary" style={{ marginTop: '1rem' }}>
                  Modifier mes critères
                </button>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                {results.map((rec, i) => {
                  const opColor = getOperatorColor(rec.offer?.operator?.slug || '')
                  const medals = ['🥇', '🥈', '🥉']
                  const scoreColor = rec.score >= 75 ? '#4ade80' : rec.score >= 50 ? '#f59e0b' : '#f87171'
                  return (
                    <div key={i} className="offer-card" style={{ padding: '1.5rem' }}>
                      <div style={{ display: 'flex', gap: '1.25rem', alignItems: 'flex-start' }}>
                        {/* Medal & Score */}
                        <div style={{ textAlign: 'center', flexShrink: 0 }}>
                          <div style={{ fontSize: '2rem', lineHeight: 1 }}>{medals[i]}</div>
                          <div style={{
                            marginTop: 8, fontSize: '1.25rem', fontWeight: 900, color: scoreColor,
                          }}>
                            {rec.score}%
                          </div>
                          <div style={{ fontSize: 10, color: 'var(--text-secondary)' }}>match</div>
                        </div>

                        {/* Offer info */}
                        <div style={{ flex: 1 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                            <span style={{
                              fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 50,
                              background: `${opColor}20`, border: `1px solid ${opColor}50`, color: opColor,
                            }}>
                              {rec.offer?.operator?.name}
                            </span>
                            <h3 style={{ fontSize: '1rem', fontWeight: 700 }}>{rec.offer?.name}</h3>
                          </div>

                          <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
                            <span style={{ fontSize: '1.5rem', fontWeight: 900 }}>
                              {formatDA(rec.offer?.priceDA)}
                            </span>
                            <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
                              <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
                                <Wifi size={12} style={{ display: 'inline', marginRight: 4 }}/>
                                {formatData(rec.offer?.dataGB)}
                              </span>
                              <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
                                <Phone size={12} style={{ display: 'inline', marginRight: 4 }}/>
                                {formatMinutes(rec.offer?.voiceMinutes)}
                              </span>
                            </div>
                          </div>

                          {/* Score bar */}
                          <div className="progress-bar" style={{ marginBottom: '1rem' }}>
                            <div className="progress-fill" style={{ width: `${rec.score}%`, background: `linear-gradient(90deg, ${scoreColor}80, ${scoreColor})` }}/>
                          </div>

                          {/* Reasons */}
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                            {rec.matchReasons.slice(0, 2).map((r, ri) => (
                              <div key={ri} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#4ade80' }}>
                                <CheckCircle size={12}/> {r}
                              </div>
                            ))}
                            {rec.mismatches.slice(0, 1).map((m, mi) => (
                              <div key={mi} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#f87171' }}>
                                <AlertCircle size={12}/> {m}
                              </div>
                            ))}
                          </div>

                          {/* Savings */}
                          {rec.savings > 0 && (
                            <div style={{
                              marginTop: '0.875rem', display: 'inline-flex', alignItems: 'center', gap: 6,
                              padding: '0.35rem 0.75rem', borderRadius: 8,
                              background: 'rgba(74,222,128,0.1)', border: '1px solid rgba(74,222,128,0.25)',
                              color: '#4ade80', fontSize: 12, fontWeight: 600,
                            }}>
                              <TrendingUp size={12}/>
                              Économie potentielle: ~{formatDA(rec.savings * 12)}/an
                            </div>
                          )}
                        </div>
                      </div>
                    </div>
                  )
                })}
              </div>
            )}

            <div style={{ display: 'flex', gap: 10, marginTop: '2rem' }}>
              <button onClick={() => setStep(1)} className="btn-ghost" style={{ flex: 1, borderRadius: 12, padding: '0.875rem' }}>
                ← Modifier mes critères
              </button>
              <Link href="/compare" style={{
                flex: 1, borderRadius: 12, padding: '0.875rem', fontSize: 14, fontWeight: 600,
                background: 'linear-gradient(135deg, #4f7fff, #7c3aed)',
                color: 'white', textDecoration: 'none', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center',
              }}>
                Comparer côte à côte →
              </Link>
            </div>
          </div>
        )}
      </div>
    </div>
  )
}
