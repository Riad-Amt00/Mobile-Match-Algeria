...
| **Search** | `src/app/api/offers/route.ts`, `src/lib/search-tokens.ts`, `src/lib/search-fts.ts` | regex token parser → Prisma `where` + SQLite FTS5 (BM25); "Read as" chips. Free-text words (e.g. `youtube`, `facebook`) hit the FTS index over name/features — verified 0 false positives; the matched feature is highlighted on each card (`OfferCard.highlight`) so app/perk hits buried in long/Arabic features are visible. Help popover lists the perk search (`search.helpText`) |
